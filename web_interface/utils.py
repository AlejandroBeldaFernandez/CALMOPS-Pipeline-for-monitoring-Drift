        ...
        try:
            with open(control_file, "r") as f:
                for line in f:
                    ...
                    if len(partes) == 2:
                        registros[partes[0]] = float(partes[1])
        except Exception as e:
            print(f"[ERROR] No se pudieron leer registros: {e}")
    return registros

def actualizar_registro(control_file: Path, archivo: str, mtime: float):
    with open(control_file, "a") as f:
        f.write(f"{archivo},{mtime}\n")
    print(f"[CONTROL] Registrado {archivo} en {control_file}")
...
def leer_metrics(metrics_path: Path):
    if metrics_path.exists():
        try:
            with open(metrics_path, "r") as f:
                return json.load(f)
        except Exception as e:
            print(f"[ERROR] No se pudieron leer métricas: {e}")
    return {}
