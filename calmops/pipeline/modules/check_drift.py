...
def check_drift(X, y, detector, logger, perf_thresholds, model_filename,
                data_dir, output_dir, control_dir, model_dir, current_filename, prediction_only=False):
    """
    ...
    Args:
        ...
        current_filename: The name of the current file being processed.
    ...
    logger.debug(f"perf_results_current BEFORE manual correction: {perf_results_current}")
    ...
    for metric, results in perf_results_current.items():
        ...
        if value is not None and threshold is not None:
            ...
            logger.info(f"Correcting metric '{metric}': value={value}, threshold={threshold}, drift -> {has_drift}")
            results["drift"] = has_drift
            perf_flags_current[metric] = has_drift
    
    logger.debug(f"perf_results_current AFTER manual correction: {perf_results_current}")
    ...
    if perf_flags_current:
        ...
        current_perf_drift = (num_drifted / num_metrics) >= 0.5 if num_metrics > 0 else False
    ...
    try:
        ...
        metrics_to_save = {}
        for metric, results in perf_results_current.items():
            value_key = next((k for k in results if k != 'threshold' and k != 'drift'), None)
            if value_key:
    ...
    if os.path.exists(control_file):
        ...
        if lines:
            ...
            if os.path.exists(ref_path):
                ...
                logger.warning(f"Reference file '{last_ref_filename}' from control_file not found in data_dir.")
    else:
