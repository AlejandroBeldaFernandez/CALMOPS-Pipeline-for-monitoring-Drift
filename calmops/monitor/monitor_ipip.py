...
def _write_runner_script(
    ...
    runner_path = pipeline_dir / f"run_{pipeline_name}_ipip.py"
...
def _launch_with_pm2(
    ...
    # Paths and names
    eco_path = base_dir / "pipelines" / pipeline_name / "ecosystem.ipip.config.js"
...
"""
    ...
    # Try to configure auto-start on Linux/macOS
    if os.name != "nt":
        try:
        ...
        logger.info(
            "[PM2] Windows detected: automatic startup on boot is not configured."
        )
...
def start_monitor_ipip(
    *,
    ...
    def execute_pipeline(file: str):
        ...
        # If a specific target_file is set, ignore all other files
        if target_file and file != target_file:
            log.info(
                f"[MONITOR] Skipping {file} because target_file is set to {target_file}"
            )
            return
