...
class ClinicGenerator:
    ...
    def __init__(self, seed=42):
        """
        Initializes the ClinicGenerator with a given random seed for reproducibility.
        """
        np.random.seed(seed)

    def _generate_module_data(self, n_samples, marginals_list, sigma_module, is_gene_module=False, n_gene_vars=0):
        """
        ...
        Z_mod = np.random.multivariate_normal(mean=mean_vec, cov=sigma_module, size=n_samples)
    ...
    def generate_demographic_data(self, n_samples: int, 
        ...
        default_marginals = {
            'Edad': stats.norm(loc=60, scale=12),
            'Sexo': stats.binom(n=1, p=0.52),
            'Propension': stats.norm() # Propensity score for group assignment
        }
        ...
        if custom_demographic_columns:
            for col_name, distribution_spec in custom_demographic_columns.items():
                if isinstance(distribution_spec, dict):
                    ...
                    params = {k: v for k, v in distribution_spec.items() if k != 'distribution'}
                    ...
                    if dist_type == 'norm':
                        ...
                        raise ValueError(f"Unsupported distribution type '{dist_type}' for demographic column '{col_name}'.")
                else:
                    # Otherwise, assume it's a scipy.stats distribution object
                    marginals_to_use[col_name] = distribution_spec
        
        if 'Propension' not in marginals_to_use:
            marginals_to_use['Propension'] = stats.norm()
        ...
        if demographic_correlations is None:
            ...
            if demographic_correlations.shape != (n_mod_vars, n_mod_vars):
                raise ValueError(f"demographic_correlations matrix shape {demographic_correlations.shape} does not match the number of demographic variables ({n_mod_vars}).")
        
        X_demo_raw = self._generate_module_data(n_samples, marginals_list, demographic_correlations)
        df_temp = pd.DataFrame(X_demo_raw, columns=ordered_col_names)

        if 'Edad' in df_temp.columns:
            df_temp['Edad'] = np.round(df_temp['Edad']).astype(int)
        if 'Sexo' in df_temp.columns:
            df_temp['Sexo_Binario'] = np.round(df_temp['Sexo']).astype(int)
            df_temp['Sexo'] = df_temp['Sexo_Binario'].map({0: 'Femenino', 1: 'Masculino'})

        patient_ids = [f'PAC_{np.random.randint(10000, 99999)}_{i}' for i in range(n_samples)]
        df_temp['ID_Paciente'] = patient_ids
        df_temp = df_temp.set_index('ID_Paciente')
        ...
        if 'Sexo' in raw_demographic_data.columns and 'Sexo_Binario' in raw_demographic_data.columns:
            raw_demographic_data = raw_demographic_data.drop(columns=['Sexo'])

        # Clean up final demographic df
        df_temp = df_temp.drop(columns=['Grupo_Binario', 'Propension'])
        if 'Sexo_Binario' in df_temp.columns:
            df_temp = df_temp.drop(columns=['Sexo_Binario'])

        return df_temp, raw_demographic_data

    def _prepare_demographic_context(self, demographic_df, demographic_id_col, raw_demographic_data,
                                     n_samples_default=100, control_disease_ratio=0.5,
                                     use_correlation=False):
        """
        Helper to extract or generate demographic context for omics data generation.
        """
        if demographic_df is not None:
            if demographic_id_col is None or (demographic_id_col not in demographic_df.columns and demographic_id_col != demographic_df.index.name):
                raise ValueError("demographic_id_col must be provided and exist in demographic_df when demographic_df is provided.")
            ...
            groups = demographic_df['Grupo'].values if 'Grupo' in demographic_df.columns else np.array(['Control'] * n_samples)
            
            if use_correlation and raw_demographic_data is None:
                raise ValueError("raw_demographic_data must be provided when correlation is used.")

            demographic_marginals_for_corr = []
            if raw_demographic_data is not None:
                for col in raw_demographic_data.columns:
                    if col != demographic_id_col and col != 'Grupo':
                        if pd.api.types.is_numeric_dtype(raw_demographic_data[col]):
                            if raw_demographic_data[col].nunique() <= 2:
                                ...
                                demographic_marginals_for_corr.append(stats.binom(n=1, p=p_val))
                            else:
                                loc_val = raw_demographic_data[col].mean()
                                scale_val = raw_demographic_data[col].std()
                                demographic_marginals_for_corr.append(stats.norm(loc=loc_val, scale=scale_val))
        else:
            n_samples = n_samples_default
            patient_ids = np.array([f'PAC_{np.random.randint(10000, 99999)}_{i}' for i in range(n_samples)])
            n_control = int(n_samples * control_disease_ratio)
            n_disease = n_samples - n_control
            groups = np.array(['Control'] * n_control + ['Enfermedad'] * n_disease)
        ...
        idx_disease = np.where(groups == 'Enfermedad')[0]

        return patient_ids, groups, idx_control, idx_disease, demographic_marginals_for_corr
    ...
    def generate_gene_data(self, 
        ...
        patient_ids, groups, idx_control, idx_disease, _ = \
            self._prepare_demographic_context(
                demographic_df, demographic_id_col, raw_demographic_data,
                n_samples_default=n_samples, control_disease_ratio=control_disease_ratio,
        ...
        if demographic_gene_correlations is not None:
            raise NotImplementedError("Demographic-gene correlations are not supported with this effect model.")
        
        X_genes_base = self._generate_module_data(n_total_samples, base_gene_marginals, 
                                                gene_correlations if gene_correlations is not None else np.identity(n_genes))
        
        df_genes = pd.DataFrame(X_genes_base, columns=[f'G_{i}' for i in range(n_genes)], index=patient_ids)
    ...
    def generate_protein_data(self,
        ...
        """
        # --- 1. Handle Demographic Data ---
        patient_ids, groups, idx_control, idx_disease, demographic_marginals_for_corr = \
            self._prepare_demographic_context(
                demographic_df, demographic_id_col, raw_demographic_data,
                n_samples_default=n_samples, control_disease_ratio=control_disease_ratio,
        ...
        if demographic_protein_correlations is not None:
            raise NotImplementedError("Stochastic per-patient effects with demographic-protein correlations are not yet implemented.")

        X_proteins_base = self._generate_module_data(n_total_samples, 
                                                     base_protein_marginals, 
                                                     protein_correlations if protein_correlations is not None else np.identity(n_proteins))
        
        df_proteins = pd.DataFrame(X_proteins_base, 
        ...
        if disease_effects_config and len(idx_disease) > 0:
            ...
            for effect in disease_effects_config:
                ...
                if not all(key in effect for key in required_keys):
                    raise ValueError(f"Invalid disease effect config. Missing keys in {effect}.")
                ...
                if isinstance(effect_value, list) and len(effect_value) == 2:
                    patient_offsets = np.random.uniform(effect_value[0], effect_value[1], size=n_disease)
                else:
                    scale = abs(effect_value) * 0.1 + 1e-6
                    patient_offsets = np.random.normal(loc=effect_value, scale=scale, size=n_disease)
                ...
                if effect_type == "additive_shift":
                    ...
                    df_proteins.loc[disease_patient_ids, protein_cols_to_affect] *= fold_changes[:, np.newaxis]
                elif effect_type == "variance_scale":
                    print(f"Warning: 'variance_scale' is not supported with the per-patient stochastic effect model for proteins. Skipping effect '{effect['name']}'.")
                    continue
                else:
                    raise ValueError(f"Unsupported effect_type '{effect_type}' in per-patient stochastic model for proteins.")

        return df_proteins

    def generate_custom_correlated_omics_data(self,
        ...
        if omics_type in ["genes", "both"] and gene_type not in ["RNA-Seq", "Microarray"]:
            raise ValueError("gene_type must be 'RNA-Seq' or 'Microarray' if omics_type is 'genes' or 'both'.")
        ...
        if omics_type == "both":
            raise NotImplementedError("Handling 'both' omics_type is not yet implemented for custom correlations.")
            ...
            for col, value in patient_filter.items():
                if col in filtered_patients_df.columns:
                    if isinstance(value, (list, np.ndarray)):
                        filtered_patients_df = filtered_patients_df[filtered_patients_df[col].isin(value)]
                    else:
                        filtered_patients_df = filtered_patients_df[filtered_patients_df[col] == value]
                else:
                    raise ValueError(f"Filter column '{col}' not found in demographic_df.")
        
        patient_ids_to_modify = filtered_patients_df.index.intersection(omics_data_df.index)
        ...
        if correlation_matrix.shape != (n_omics_subset, n_omics_subset):
            raise ValueError(f"correlation_matrix shape {correlation_matrix.shape} does not match subset size ({n_omics_subset}).")
        ...
        for i, omics_idx in enumerate(omics_subset_indices):
            ...
            if omics_type == "genes":
                if gene_type == "Microarray":
                    ...
                    if disease_effect_type == "additive_shift" and disease_effect_value is not None:
                        disease_dist = stats.norm(loc=loc + disease_effect_value, scale=scale)
                    ...
                    if variance > mean and mean > 0:
                        ...
                        if r > 0 and 0 < p < 1:
                            ...
                            control_marginals_subset.append(base_dist)
                            if disease_effect_type == "fold_change" and disease_effect_value is not None:
                                new_mean = mean * disease_effect_value
                                new_p = r / (r + new_mean)
                                if r > 0 and 0 < new_p < 1:
                                    disease_marginals_subset.append(stats.nbinom(n=r, p=new_p))
                                else:
                                    disease_marginals_subset.append(base_dist) # Fallback
                            else:
                                disease_marginals_subset.append(base_dist)
                        else:
                ...
                if not log_data.empty:
                    ...
                    control_marginals_subset.append(base_dist)
                    if disease_effect_type == "additive_shift" and disease_effect_value is not None:
                        disease_dist = stats.lognorm(s=s, scale=np.exp(loc + disease_effect_value))
                        disease_marginals_subset.append(disease_dist)
                    else:
                        disease_marginals_subset.append(base_dist)
                else:
        ...
        marginals_to_use = disease_marginals_subset if (disease_effect_type and disease_effect_value is not None) else control_marginals_subset
        ...
        updated_omics_data_df.loc[patient_ids_to_modify, updated_omics_data_df.columns[omics_subset_indices]] = generated_data_subset

        return updated_omics_data_df

    def inject_drift_group_transition(self,
                                      demographic_df: pd.DataFrame,
                                      omics_data_df: pd.DataFrame,
        ...
        if transition_type not in ['control_to_disease', 'disease_to_control', 'bidirectional']:
            raise ValueError("transition_type must be one of 'control_to_disease', 'disease_to_control', 'bidirectional'.")

        def get_patient_ids_for_transition(source_group: str):
            source_patients = updated_demographic_df[updated_demographic_df['Grupo'] == source_group].index.tolist()
            num_to_transition = 0
            if 'percentage' in selection_criteria:
                num_to_transition = int(len(source_patients) * selection_criteria['percentage'])
            elif 'random' in selection_criteria:
                num_to_transition = min(selection_criteria['random'], len(source_patients))
            elif 'patient_ids' in selection_criteria:
                return [pid for pid in selection_criteria['patient_ids'] if pid in source_patients]
            
            return np.random.choice(source_patients, num_to_transition, replace=False).tolist()

        patient_ids_to_modify = []
        if transition_type == 'control_to_disease' or transition_type == 'bidirectional':
            patient_ids_to_modify.extend(get_patient_ids_for_transition('Control'))
        if transition_type == 'disease_to_control' or transition_type == 'bidirectional':
            patient_ids_to_modify.extend(get_patient_ids_for_transition('Enfermedad'))
        ...
        for patient_id in patient_ids_to_modify:
            current_group = updated_demographic_df.loc[patient_id, 'Grupo']
            new_group = 'Enfermedad' if current_group == 'Control' else 'Control'
            updated_demographic_df.loc[patient_id, 'Grupo'] = new_group
            if 'Grupo_Binario' in updated_demographic_df.columns:
                updated_demographic_df.loc[patient_id, 'Grupo_Binario'] = 1 if new_group == 'Enfermedad' else 0

        # Regenerate omics data for all transitioned patients
        if patient_ids_to_modify:
            control_to_disease_ids = [pid for pid in patient_ids_to_modify if updated_demographic_df.loc[pid, 'Grupo'] == 'Enfermedad']
            disease_to_control_ids = [pid for pid in patient_ids_to_modify if updated_demographic_df.loc[pid, 'Grupo'] == 'Control']

            # Process transitions to Disease
            if control_to_disease_ids:
                temp_demographic_df_d = updated_demographic_df.loc[control_to_disease_ids].reset_index()
                id_col_d = temp_demographic_df_d.columns[0]

                if omics_type == 'genes' or omics_type == 'both':
                    gene_effects = []
                    if disease_gene_indices and disease_effect_type and disease_effect_value is not None:
                        gene_effects.append({
                    ...
                    gene_df_d = self.generate_gene_data(
                        n_genes=n_genes_total, gene_type=gene_type, demographic_df=temp_demographic_df_d,
                        demographic_id_col=id_col_d, disease_effects_config=gene_effects)
                    gene_cols = gene_df_d.columns
                    updated_omics_data_df.loc[control_to_disease_ids, gene_cols] = gene_df_d.loc[control_to_disease_ids, gene_cols].values

                if omics_type == 'proteins' or omics_type == 'both':
                    protein_effects = []
                    if disease_protein_indices and disease_effect_type and disease_effect_value is not None:
                    ...
                    protein_df_d = self.generate_protein_data(
                        n_proteins=n_proteins_total, demographic_df=temp_demographic_df_d,
                        demographic_id_col=id_col_d, disease_effects_config=protein_effects)
                    protein_cols_d = protein_df_d.columns
                    updated_omics_data_df.loc[control_to_disease_ids, protein_cols_d] = protein_df_d.loc[control_to_disease_ids, protein_cols_d].values

            # Process transitions to Control
            if disease_to_control_ids:
                temp_demographic_df_c = updated_demographic_df.loc[disease_to_control_ids].reset_index()
                id_col_c = temp_demographic_df_c.columns[0]

                if omics_type == 'genes' or omics_type == 'both':
                    gene_df_c = self.generate_gene_data( 
                        n_genes=n_genes_total, gene_type=gene_type, demographic_df=temp_demographic_df_c,
                        demographic_id_col=id_col_c, disease_effects_config=[]) 
                    gene_cols_c = gene_df_c.columns
                    updated_omics_data_df.loc[disease_to_control_ids, gene_cols_c] = gene_df_c.loc[disease_to_control_ids, gene_cols_c].values

                if omics_type == 'proteins' or omics_type == 'both':
                    protein_df_c = self.generate_protein_data(
                        n_proteins=n_proteins_total, demographic_df=temp_demographic_df_c,
                        demographic_id_col=id_col_c, disease_effects_config=[])
                    protein_cols_c = protein_df_c.columns
                    updated_omics_data_df.loc[disease_to_control_ids, protein_cols_c] = protein_df_c.loc[disease_to_control_ids, protein_cols_c].values

        return updated_demographic_df, updated_omics_data_df
        
    def inject_drift_correlated_modules(self,
                                        omics_data_df: pd.DataFrame,
                                        module_indices: list,
        ...
        if remove_indices:
            module_indices = [idx for idx in module_indices if idx not in remove_indices]
        if add_indices:
        ...
        if new_correlation_matrix is None:
            ...
            new_correlation_matrix = updated_omics_df.iloc[:, module_indices].corr().values
        ...
        updated_omics_df = self.generate_custom_correlated_omics_data(
            demographic_df=pd.DataFrame(index=updated_omics_df.index), # Pass demographic_df to get patient IDs
    ...
    def generate_additional_time_step_data(self,
                                           n_samples: int,
                                           date_value: str,
        ...
        custom_gene_params = parameter_drift_config.get('custom_gene_parameters', {}) if parameter_drift_config else {}
        custom_protein_params = parameter_drift_config.get('custom_protein_parameters', {}) if parameter_drift_config else {}
    ...
    def _summarize_longitudinal_transition(self, df_demo_t2, idx_transicion, df_genes_t1, df_genes_t2, df_proteins_t1, df_proteins_t2, gene_indices, protein_indices):
        """Helper to summarize changes in the transition cohort."""
        summary_stream = io.StringIO()
        summary_stream.write("--- LONGITUDINAL TRANSITION (DRIFT) COHORT ANALYSIS ---\n")
        summary_stream.write(f"Number of patients transitioned: {len(idx_transicion)}\n")
        
        if len(idx_transicion) > 0:
            summary_stream.write("Gene expression changes for transitioned patients (Module A):\n")
            gene_cols = df_genes_t1.columns[gene_indices]
            gene_changes = (df_genes_t2.loc[idx_transicion, gene_cols].mean() - 
                            df_genes_t1.loc[idx_transicion, gene_cols].mean())
            summary_stream.write(f"Mean gene changes:\n{gene_changes.to_string()}\n\n")

            summary_stream.write("Protein expression changes for transitioned patients (Module A):\n")
            protein_cols = df_proteins_t1.columns[protein_indices]
            protein_changes = (df_proteins_t2.loc[idx_transicion, protein_cols].mean() - 
                               df_proteins_t1.loc[idx_transicion, protein_cols].mean())
            summary_stream.write(f"Mean protein changes:\n{protein_changes.to_string()}\n")
...
def replicate_genes_proteinas(generator: ClinicGenerator, mode, output_dir, n_samples=100, factor_escala=1.0, gene_mean_loc_center=7.0, gene_mean_log_center=np.log(80)):
    """
    Runs a specific T1 vs T2 longitudinal simulation using the ClinicGenerator.
    ...
    df_demo_t1, raw_demo_t1 = generator.generate_demographic_data(n_samples, control_disease_ratio=0.5)
    ...
    if mode == 'microarray':
        gene_effects_config = [
            {'name': 'Module_A', 'indices': gene_indices_modA, 'effect_type': 'additive_shift', 'effect_value': [0.8, 1.2]},
            {'name': 'Module_B', 'indices': gene_indices_modB, 'effect_type': 'variance_scale', 'effect_value': [1.5, 2.0]},
            {'name': 'Module_C', 'indices': gene_indices_modC, 'effect_type': 'additive_shift', 'effect_value': [-0.6, -0.4]}
        ]
    else: # rnaseq
        gene_effects_config = [
            {'name': 'Module_A', 'indices': gene_indices_modA, 'effect_type': 'fold_change', 'effect_value': [2.0, 3.0]},
            {'name': 'Module_B', 'indices': gene_indices_modB, 'effect_type': 'fold_change', 'effect_value': [0.5, 0.7]},
            # Module C has no effect in this scenario
        ]

    protein_effects_config = [
        {'name': 'Module_A', 'indices': protein_indices_modA, 'effect_type': 'additive_shift', 'effect_value': [np.log(1.8), np.log(2.2)]},
        {'name': 'Module_B', 'indices': protein_indices_modB, 'effect_type': 'variance_scale', 'effect_value': [1.2, 1.5]}
    ]
    ...
    def fill_block(matrix, indices, corr_value):
        for i in indices:
            for j in indices:
                if i < j:
                    matrix[i, j] = matrix[j, i] = np.random.uniform(corr_value[0], corr_value[1])
    ...
    fill_block(gene_correlations, gene_indices_modA, [0.6, 0.8])    # High correlation within Module A
    fill_block(gene_correlations, gene_indices_modB, [0.5, 0.7])    # High correlation within Module B
    fill_block(gene_correlations, gene_indices_modC, [0.3, 0.5])    # Low correlation within Module C
    fill_inter_block(gene_correlations, gene_indices_modA, gene_indices_modB, [0.2, 0.4]) # Medium correlation between A and B
    ...
    df_genes_t1 = generator.generate_gene_data(
        n_genes=n_genes, gene_type=mode, demographic_df=df_demo_t1,
        demographic_id_col=df_demo_t1.index.name, gene_correlations=gene_correlations,
        disease_effects_config=gene_effects_config, gene_mean_loc_center=gene_mean_loc_center,
        gene_mean_log_center=gene_mean_log_center)

    df_proteins_t1 = generator.generate_protein_data(
        n_proteins=n_proteins, demographic_df=df_demo_t1,
        demographic_id_col=df_demo_t1.index.name, protein_correlations=protein_correlations,
    ...
    df_list_t1 = [("DEMOGRAPHIC T1", df_demo_t1.copy()), (f"GENES T1", df_genes_t1.copy()), ("PROTEINS T1", df_proteins_t1.copy())]
    report_t1 = generator._generate_text_report(df_list_t1, report_title_t1)
    
    with open(os.path.join(output_dir, f"report_t1_{mode}.txt"), 'w', encoding='utf-8') as f:
        f.write(report_t1)

    df_demo_t1.to_csv(os.path.join(output_dir, f"dataset_demografico_t1_{mode}.csv"), index=True)
    df_genes_t1.to_csv(os.path.join(output_dir, f"dataset_genes_t1_{mode}.csv"), index=True)
    df_proteins_t1.to_csv(os.path.join(output_dir, f"dataset_proteinas_t1_{mode}.csv"), index=True)
    ...
    idx_control_t1 = df_demo_t1[df_demo_t1['Grupo'] == 'Control'].index
    n_control_to_disease = len(idx_control_t1) // 2
    idx_transicion = np.random.choice(idx_control_t1, n_control_to_disease, replace=False)

    df_demo_t2 = df_demo_t1.copy()
    df_demo_t2.loc[idx_transicion, 'Grupo'] = 'Enfermedad'
    
    print(f"T2 Cohort: {len(df_demo_t2[df_demo_t2['Grupo'] == 'Control'])} control, {len(df_demo_t2[df_demo_t2['Grupo'] == 'Enfermedad'])} disease.")
    ...
    df_genes_t2 = generator.generate_gene_data(
        n_genes=n_genes, gene_type=mode, demographic_df=df_demo_t2, 
        demographic_id_col=df_demo_t2.index.name, gene_correlations=gene_correlations,
        disease_effects_config=gene_effects_config, gene_mean_loc_center=gene_mean_loc_center,
        gene_mean_log_center=gene_mean_log_center)

    df_proteins_t2 = generator.generate_protein_data(
        n_proteins=n_proteins, demographic_df=df_demo_t2, 
        demographic_id_col=df_demo_t2.index.name, protein_correlations=protein_correlations,
        disease_effects_config=protein_effects_config)

    # --- 7. T2 Report and Save ---
    report_title_t2 = f"T2 DATA ({mode.upper()}) - WITH LONGITUDINAL DRIFT"
    df_list_t2 = [("DEMOGRAPHIC T2", df_demo_t2.copy()), (f"GENES T2", df_genes_t2.copy()), (f"PROTEINS T2", df_proteins_t2.copy())]
    report_t2 = generator._generate_text_report(df_list_t2, report_title_t2)
    
    # For the summary, we can provide all affected indices
    all_gene_indices = sorted(list(set(gene_indices_modA + gene_indices_modB + gene_indices_modC)))
    all_protein_indices = sorted(list(set(protein_indices_modA + protein_indices_modB + protein_indices_modC)))

    transition_summary = generator._summarize_longitudinal_transition(
        df_demo_t2, idx_transicion, 
        df_genes_t1, df_genes_t2, 
        df_proteins_t1, df_proteins_t2, 
        all_gene_indices, all_protein_indices
    )
    report_t2 += "\n\n" + transition_summary

    with open(os.path.join(output_dir, f"report_t2_{mode}.txt"), 'w', encoding='utf-8') as f:
        f.write(report_t2)

    df_demo_t2.to_csv(os.path.join(output_dir, f"dataset_demografico_t2_{mode}.csv"), index=True)
    df_genes_t2.to_csv(os.path.join(output_dir, f"dataset_genes_t2_{mode}.csv"), index=True)
    df_proteins_t2.to_csv(os.path.join(output_dir, f"dataset_proteinas_t2_{mode}.csv"), index=True)
