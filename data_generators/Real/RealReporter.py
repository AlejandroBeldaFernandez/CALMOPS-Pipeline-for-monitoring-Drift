...
class RealReporter:
    ...
    def _report_block_analysis(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, block_column: str, target_column: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for block_id in unique_blocks:
                ...
                if target_column and target_column in real_block_df.columns:
                    block_stats["real_target_distribution"] = real_block_df[target_column].value_counts(normalize=True).to_dict()
                if target_column and target_column in synthetic_block_df.columns:
                    block_stats["synthetic_target_distribution"] = synthetic_block_df[target_column].value_counts(normalize=True).to_dict()
    ...
    def generate_comprehensive_report(
        ...
        report = {
            ...
            "target_analysis": self._report_target_analysis(real_df, synthetic_df, target_column) if target_column else None,
            "sdv_quality": self._report_sdv_quality_assessment(real_df, synthetic_df) if SDV_AVAILABLE else None,
        ...
        if drift_config:
            ...
            if "target_col" in drift_config and drift_config["target_col"] not in affected_cols:
        ...
        if final_time_col and final_time_col not in real_df.columns:
            self.logger.warning(f"Provided time column '{final_time_col}' not found. Ignoring.")
            final_time_col = None

        if not final_time_col: # If not provided or not found, try to find default ones
            final_time_col = 'timestamp' if 'timestamp' in real_df.columns else 'chunk' if 'chunk' in real_df.columns else None

        # --- Generate and Save Plots --- #
        plots = {}
        # Generic plots (PCA, Correlation) are not focused by default unless specified by user
        plots['pca_comparison'] = self._save_pca_plot(real_df, synthetic_df, output_dir, generator_name)
        plots['correlation_heatmap'] = self._save_correlation_heatmap(real_df, synthetic_df, output_dir, focus_cols=focus_cols)
        plots['interaction_plots'] = self._save_interaction_plots(real_df, synthetic_df, output_dir, focus_cols=focus_cols)
        plots['mode_comparison'] = self._save_mode_analysis_plot(report["mode_analysis"], output_dir)
        ...
        for col in cols_for_dist_plots:
            if col in synthetic_df.columns:
                if col == final_time_col:
                    continue
                dist_plots[col] = self._save_distribution_plot(real_df, synthetic_df, col, output_dir)
                
                is_categorical = False
                if pd.api.types.is_string_dtype(real_df[col]) or pd.api.types.is_categorical_dtype(real_df[col]):
                    is_categorical = True
                ...
                if is_categorical:
                    balance_plots[col] = self._save_instances_balance_plot(real_df, synthetic_df, col, output_dir)

                if pd.api.types.is_numeric_dtype(real_df[col]):
                    box_plots[col] = self._save_boxplot_plot(real_df, synthetic_df, col, output_dir)
        ...
        plots['time_evolution_plots'] = self._save_time_evolution_plots(
            real_df, synthetic_df, output_dir, time_col=final_time_col, focus_cols=focus_cols, drift_config=drift_config
        )
        ...
        if block_column and block_column in synthetic_df.columns:
            ...
            for block_id in unique_blocks:
                ...
                if real_block_df.empty or synthetic_block_df.empty:
                    self.logger.warning(f"Skipping plots for block {block_id} due to empty data.")
                    continue
                ...
                for col in real_block_df.columns:
                    ...
                    plot_path = self._save_distribution_plot(real_block_df, synthetic_block_df, col, block_output_dir)
                    if plot_path:
                        block_plots_report[str(block_id)]["distribution_plots"][col] = plot_path

                    # Check if categorical to generate balance plot
                    is_categorical = False
                    if pd.api.types.is_string_dtype(real_block_df[col]) or pd.api.types.is_categorical_dtype(real_block_df[col]):
                        is_categorical = True
                    elif pd.api.types.is_numeric_dtype(real_block_df[col]) and real_block_df[col].nunique() < 25:
                        is_categorical = True
                    
                    if is_categorical:
                        balance_plot_path = self._save_instances_balance_plot(real_block_df, synthetic_block_df, col, block_output_dir)
                        if balance_plot_path:
                            block_plots_report[str(block_id)]["instances_balance_plots"][col] = balance_plot_path
                
                if self.verbose:
                    print(f"  -> Plots for block '{block_id}' saved to: {block_output_dir}")
    ...
    def _get_weighted_sdv_score(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, base_score: float) -> float:
        """
        ...
        duplicated_rows_percentage = total_bad_rows / len(synthetic_df) if len(synthetic_df) > 0 else 0.0
        
        # --- Calculate null percentage ---
        null_values_percentage = synthetic_df.isnull().sum().sum() / (synthetic_df.shape[0] * synthetic_df.shape[1]) if synthetic_df.size > 0 else 0.0
        
        # --- Calculate final score ---
        base_score = 0.0 if pd.isna(base_score) else base_score
        weighted_score = base_score * (1 - duplicated_rows_percentage) * (1 - null_values_percentage)
        
        if self.verbose:
            self.logger.info(f"Weighted Score Calculation: Base={base_score:.3f}, Duplication Penalty={duplicated_rows_percentage:.3f}, Null Penalty={null_values_percentage:.3f} -> Final={weighted_score:.3f}")

        return weighted_score

    def _save_target_distribution_plot(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, target_column: str, output_dir: str) -> Optional[str]:
        """
        ...
        try:
            ...
            problem_type = ""
            if pd.api.types.is_string_dtype(real_target) or pd.api.types.is_categorical_dtype(real_target):
            ...
            if problem_type == "Regression":
                self.logger.info(f"Target '{target_column}' is regression. Skipping target distribution plot.")
                return None
            ...
            df_counts = pd.DataFrame({'Real': real_counts, 'Synthetic': synthetic_counts}).fillna(0).sort_index()
            ...
            plt.tight_layout()
            plot_path = os.path.join(output_dir, f"target_dist_plot_{target_column}.png")
            fig.savefig(plot_path)
            plt.close(fig)
            return plot_path
        except Exception as e:
            self.logger.error(f"Failed to generate target distribution plot for {target_column}: {e}")
            return None

    def _save_instances_balance_plot(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, column: str, output_dir: str) -> Optional[str]:
        """
        ...
        try:
            ...
            if len(df_counts) > 20:
                self.logger.warning(f"High cardinality in column '{column}' ({len(df_counts)} unique values). Plot may be cluttered.")
            ...
            self.logger.error(f"Failed to generate instance balance plot for {column}: {e}")
            return None

    def _save_time_evolution_plots(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, output_dir: str, time_col: Optional[str] = None, focus_cols: Optional[List[str]] = None, drift_config: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
        ...
        if drift_config:
            ...
            if "target_column" in drift_config and drift_config["target_column"] not in features_to_plot:
        ...
        for col in continuous_numeric_cols:
            ...
            try:
                ...
                if time_col:
                    ...
                    real_agg = real_df.groupby(time_col)[col].agg(['mean', 'std']).fillna(0)
                    synth_agg = synthetic_df.groupby(time_col)[col].agg(['mean', 'std']).fillna(0)

                    # Calculate moving average of the mean
                    real_mean_ma = real_agg['mean'].rolling(window=moving_avg_window, min_periods=1, center=True).mean()
                    synth_mean_ma = synth_agg['mean'].rolling(window=moving_avg_window, min_periods=1, center=True).mean()

                    # Plot moving averages
                    real_mean_ma.plot(ax=ax, label=f'Original Mean ({moving_avg_window}-MA)', style='-', linewidth=2.5, color=real_color)
                    synth_mean_ma.plot(ax=ax, label=f'Generated Mean ({moving_avg_window}-MA)', style='--', linewidth=2.5, color=synth_color)
                else:
                    # Raw plot for index-based axis with moving average
                    real_ma = real_df[col].rolling(window=moving_avg_window, min_periods=1, center=True).mean()
                    synth_ma = synthetic_df[col].rolling(window=moving_avg_window, min_periods=1, center=True).mean()

                    real_ma.reset_index(drop=True).plot(ax=ax, label=f'Original ({moving_avg_window}-MA)', style='-', linewidth=2, color=real_color)
                    synth_ma.reset_index(drop=True).plot(ax=ax, label=f'Generated ({moving_avg_window}-MA)', style='--', linewidth=2, color=synth_color)
                ...
                if drift_config:
                    ...
                    if method == "inject_feature_drift_recurrent" and "applied_windows" in drift_config:
                        applied_windows = drift_config.get("applied_windows", [])
                        unique_blocks = sorted(list(set(w.get('block_id') for w in applied_windows if w.get('block_id') is not None)))
                        palette = sns.color_palette("husl", n_colors=len(unique_blocks))
                        color_map = {block_id: color for block_id, color in zip(unique_blocks, palette)}
                        ...
                        for window in applied_windows:
                            ...
                            if not all([block_id, center_idx is not None, width is not None]):
                                continue

                            start_idx = max(0, int(center_idx) - int(width) // 2)
                            end_idx = min(len(synthetic_df) - 1, int(center_idx) + int(width) // 2)
                            ...
                            if time_col:
                                if start_idx < len(synthetic_df) and end_idx < len(synthetic_df):
                                    start_val = synthetic_df[time_col].iloc[start_idx]
                                    end_val = synthetic_df[time_col].iloc[end_idx]
                                    ax.axvspan(start_val, end_val, color=color, alpha=0.2, label=label)
                            else:
                                ax.axvspan(start_idx, end_idx, color=color, alpha=0.2, label=label)
                    ...
                    # Case 2: General window-based drifts (gradual, incremental, abrupt)
                        ...
                        color = 'orange' if method == "inject_feature_drift_incremental" else 'red'
                        ...
                        if start_idx_only is not None:
                            ...
                            if time_col:
                                if start_idx_only < len(synthetic_df):
                                    start_val = synthetic_df[time_col].iloc[start_idx_only]
                                    ax.axvline(x=start_val, color='red', linestyle='--', linewidth=2, label=label)
                            else:
                                ax.axvline(x=start_idx_only, color='red', linestyle='--', linewidth=2, label=label)
        ...
        if time_col:
            for col in all_categorical_like_cols:
                ...
                plots[col] = self._save_categorical_evolution_plot(real_df, synthetic_df, time_col, col, output_dir)
        
        return plots

    def _save_categorical_evolution_plot(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, time_col: str, category_col: str, output_dir: str) -> Optional[str]:
        ...
        try:
            ...
            if n_unique > HIGH_CARDINALITY_THRESHOLD:
                self.logger.info(f"High cardinality ({n_unique}) detected for '{category_col}'. Grouping less frequent categories into 'Other'.")
                
                # Find top categories from the real data
                top_categories = real_df_plot[category_col].value_counts().nlargest(HIGH_CARDINALITY_THRESHOLD - 1).index.tolist()
                
                # Apply the grouping to both dataframes
                real_df_plot[category_col] = real_df_plot[category_col].apply(lambda x: x if x in top_categories else 'Other')
                synthetic_df_plot[category_col] = synthetic_df_plot[category_col].apply(lambda x: x if x in top_categories else 'Other')

            # --- Data Preparation ---
            # Pivot tables to get proportions of each category over time
            real_pivot = real_df_plot.pivot_table(index=time_col, columns=category_col, aggfunc='size', fill_value=0)
            real_pivot = real_pivot.div(real_pivot.sum(axis=1), axis=0)

            synth_pivot = synthetic_df_plot.pivot_table(index=time_col, columns=category_col, aggfunc='size', fill_value=0)
            ...
            real_aligned = real_pivot.reindex(index=all_idx, columns=all_cols, fill_value=0)
            synth_aligned = synth_pivot.reindex(index=all_idx, columns=all_cols, fill_value=0)
            ...
            ax1.set_title(f'Evolution of {category_col} (Real)', fontsize=18, weight='bold')
            ax1.set_ylabel('Proportion', fontsize=14)
            ax1.legend(title=category_col, bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=12)
            ...
            ax2.set_title(f'Evolution of {category_col} (Synthetic)', fontsize=18, weight='bold')
            ax2.set_ylabel('Proportion', fontsize=14)
            ax2.legend(title=category_col, bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=12)
            ax2.grid(True, which='major', linestyle='--', linewidth='0.5', color='grey')

            # Plot 3: Absolute Error
            error_pivot.plot(kind='line', ax=ax3, marker='o', linestyle='--', markersize=5)
            ax3.set_title('Absolute Error in Proportion (Real vs. Synthetic)', fontsize=18, weight='bold')
            ...
            ax3.legend(title='Category', bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=12)
            ...
            plot_path = os.path.join(output_dir, f"categorical_evolution_{category_col}.png")
            fig.savefig(plot_path)
            plt.close(fig)
            return plot_path
        except Exception as e:
            self.logger.error(f"Failed to generate categorical evolution plot for {category_col}: {e}")
            return None

    def _save_distribution_plot(self, real_df, synthetic_df, column, output_dir) -> Optional[str]:
        ...
        try:
            ...
            if is_numeric:
                ...
                sns.kdeplot(synthetic_df[column], label='Synthetic', fill=True, alpha=0.7)
                ...
                if real_series.nunique() > HIGH_CARDINALITY_THRESHOLD:
                    top_categories = real_series.value_counts().nlargest(HIGH_CARDINALITY_THRESHOLD - 1).index.tolist()
                    real_series = real_series.apply(lambda x: x if x in top_categories else 'Other')
                    synth_series = synth_series.apply(lambda x: x if x in top_categories else 'Other')
                before_counts = real_series.value_counts(normalize=True).rename('Real')
                after_counts = synth_series.value_counts(normalize=True).rename('Synthetic')
                ...
                if len(df_counts) > CATEGORY_THRESHOLD:
                    ...
                    ax = df_counts.plot(kind='line', alpha=0.8, ax=plt.gca(), marker='o', linewidth=2, markersize=8)
                    for i, (index, row) in enumerate(df_counts.iterrows()):
                        for j, value in enumerate(row):
                            ax.text(i + (j * 0.1), value + 0.01, f'{value:.2f}', ha='center', va='bottom', fontsize=8)
    ...
    def _save_correlation_heatmap(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, output_dir: str, focus_cols: Optional[List[str]] = None) -> Optional[str]:
        ...
        try:
            ...
            if len(numeric_cols) < 2:
                self.logger.info("Not enough numeric columns to generate a correlation heatmap.")
                return None
    ...
    def _save_interaction_plots(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, output_dir: str, max_plots: int = 5, focus_cols: Optional[List[str]] = None) -> Dict[str, str]:
                ...
                for cat_col in categorical_cols:
                    ...
                    if 2 <= real_df[cat_col].nunique() <= 10:
                        try:
                            ...
                            self.logger.warning(f"Could not generate interaction plot for {cat_col} and {num_col}: {e}")
                if plot_count >= max_plots:
                    break
            return plots
        except Exception as e:
    ...
    def _save_pca_plot(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, output_dir: str, generator_name: str) -> Optional[str]:
        ...
        try:
            ...
            numeric_features = real_df_pca.select_dtypes(include=np.number).columns.tolist()
            categorical_features = real_df_pca.select_dtypes(include=['object', 'category']).columns.tolist()
            ...
            sns.scatterplot(x=real_pca[:, 0], y=real_pca[:, 1], alpha=0.5, label='Real Data')
            sns.scatterplot(x=synthetic_pca[:, 0], y=synthetic_pca[:, 1], alpha=0.5, label=f'{generator_name} (Synthetic)', marker='X')
    ...
    def _report_basic_info(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame) -> Dict[str, Any]:
        """
    ...
    def _report_target_analysis(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, target_column: str) -> Optional[Dict[str, Any]]:
        """
        ...
        if target_column not in real_df.columns or target_column not in synthetic_df.columns:
            self.logger.warning(f"Target column '{target_column}' not found in one of the dataframes.")
            return None
        try:
            report = {}
            real_target = real_df[target_column]
            ...
            if problem_type == "Classification":
                ...
                report["synthetic_data_describe"] = synthetic_target.describe().to_dict()
                ks_statistic, p_value = stats.ks_2samp(real_target.dropna(), synthetic_target.dropna())
                report["distribution_similarity"] = {
            ...
            self.logger.error(f"Failed to generate target analysis for column '{target_column}': {e}")
            return {"error": str(e)}

    def _report_sdv_quality_assessment(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame) -> Optional[Dict[str, Any]]:
        """
        ...
        if not SDV_AVAILABLE:
            self.logger.warning("SDV/SDMetrics library not found. Skipping quality assessment.")
            return {"status": "SKIPPED", "reason": "SDV/SDMetrics library is not installed."}
        if self.verbose:
        ...
        try:
            ...
            if extra_cols_in_synth:
                cols_to_drop = [col for col in extra_cols_in_synth if col in synthetic_df_eval.columns]
                if cols_to_drop:
            ...
            final_cols = [col for col in real_df_eval.columns if col in synthetic_df_eval.columns]
            ...
            if real_df_eval.empty or synthetic_df_eval.empty:
                self.logger.warning("SDV Assessment skipped: One of the dataframes is empty after column alignment.")
                return {"status": "SKIPPED", "reason": "Empty dataframe after alignment."}
            ...
            weighted_score = self._get_weighted_sdv_score(real_df_eval, synthetic_df_eval, base_score=quality_report.get_score())
            ...
            if self.verbose:
                print(f"✅ SDMetrics Assessment complete. Overall: {final_report['overall_quality_score']:.2f}, Weighted: {final_report['weighted_quality_score']:.2f}")
            return final_report
        except Exception as e:
            self.logger.error(f"SDMetrics quality assessment failed: {e}")
            return {"status": "FAILED", "error": str(e)}

    def _save_boxplot_plot(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame, column: str, output_dir: str) -> Optional[str]:
    ...
    def _report_mode_analysis(self, real_df: pd.DataFrame, synthetic_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in categorical_cols:
            if col in synthetic_df.columns:
                try:
                    real_mode_info = real_df[col].value_counts(normalize=True).nlargest(1)
                    synth_mode_info = synthetic_df[col].value_counts(normalize=True).nlargest(1)
                    ...
                    self.logger.error(f"Failed to generate mode analysis for column '{col}': {e}")
    ...
    def _save_mode_analysis_plot(self, mode_analysis_data: Dict[str, Any], output_dir: str) -> Optional[str]:
        ...
        try:
            ...
            real_proportions = [d.get('real_mode', {}).get('proportion', 0) for d in mode_analysis_data.values()]
            real_modes = [str(d.get('real_mode', {}).get('value', 'N/A')) for d in mode_analysis_data.values()]
            synth_proportions = [d.get('synthetic_mode', {}).get('proportion', 0) for d in mode_analysis_data.values()]
            synth_modes = [str(d.get('synthetic_mode', {}).get('value', 'N/A')) for d in mode_analysis_data.values()]
            ...
            bars1 = ax.barh(y + bar_height/2, df['Real Proportion'], height=bar_height, label='Real', color='skyblue', alpha=0.8)
            bars2 = ax.barh(y - bar_height/2, df['Synthetic Proportion'], height=bar_height, label='Synthetic', color='salmon', alpha=0.8)
            ...
            real_labels = [f"{df['Real Mode'][i]} ({df['Real Proportion'][i]:.2f})" for i in range(len(bars1))]
            synth_labels = [f"{df['Synthetic Mode'][i]} ({df['Synthetic Proportion'][i]:.2f})" for i in range(len(bars2))]
            ...
            if not df.empty and (df['Real Proportion'].max() > 0 or df['Synthetic Proportion'].max() > 0):
                 ax.set_xlim(right=max(df['Real Proportion'].max(), df['Synthetic Proportion'].max()) * 1.3)
    ...
    def run_scaling_evaluation(
        ...
        # --- Block-based Scaling ---
        if block_column and block_column in synthetic_data_full.columns and block_column in original_data.columns:
            if self.verbose:
                print(f"--- Starting Scaling Evaluation by Blocks ('{block_column}') ---")
            
            all_blocks = sorted(synthetic_data_full[block_column].unique())
            if len(all_blocks) <= 1:
                if self.verbose: print("Only one block found, skipping block-wise scaling evaluation.")
                return None
            ...
            for i in x_ticks:
                ...
                if self.verbose: print(f"Evaluating blocks: {blocks_to_include}...")
                
                synthetic_subset = synthetic_data_full[synthetic_data_full[block_column].isin(blocks_to_include)]
                original_subset = original_data[original_data[block_column].isin(blocks_to_include)]

                if original_subset.empty or synthetic_subset.empty:
                    self.logger.warning(f"Skipping evaluation for blocks {blocks_to_include} due to empty data.")
                    scores.append(0.0)
                    continue
                ...
                try:
                    ...
                    score = self._get_weighted_sdv_score(original_subset, synthetic_subset, quality_report.get_score())
                    scores.append(score)
                except Exception as e:
                    self.logger.error(f"Error during block scaling evaluation for blocks {blocks_to_include}: {e}")
                    scores.append(0.0)
        ...
        # --- Percentage-based Scaling (Fallback) --- #
            ...
            if self.verbose:
                print(f"Evaluating subsets of sizes: {sample_sizes} (from a total of {total_rows} synthetic rows)")
            scores = []
            for n_samples in sample_sizes:
                if self.verbose: print(f"Evaluating a subset of {n_samples} samples...")
                try:
                    synthetic_subset = synthetic_data_full.sample(n=n_samples, random_state=42)
                    if not synthetic_subset.empty:
                        ...
                        score = self._get_weighted_sdv_score(original_data, synthetic_subset, quality_report.get_score())
                        scores.append(score)
                    else:
                        scores.append(0.0)
                except Exception as e:
                    self.logger.error(f"Error during scaling evaluation for {n_samples} samples: {e}")
                    scores.append(0.0)
        ...
        plt.close(fig)
        if self.verbose: print(f"--- Scaling Evaluation Complete. Plot saved to: {plot_path} ---")
        return plot_path
