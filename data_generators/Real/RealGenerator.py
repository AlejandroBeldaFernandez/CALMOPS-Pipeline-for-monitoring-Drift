...
class RealGenerator:
    ...
    def __init__(
        ...
        valid_methods = ['cart', 'rf', 'lgbm', 'gmm', 'ctgan', 'tvae', 'copula', 'datasynth', 'resample']
        if self.method not in valid_methods:
            raise ValueError(f"Unknown synthesis method '{self.method}'. Valid methods are: {valid_methods}")
    ...
    def _get_synthesizer(self) -> Union[CTGANSynthesizer, TVAESynthesizer, CopulaGANSynthesizer]:
        ...
        if self.method == 'ctgan':
            return CTGANSynthesizer(metadata=self.metadata, epochs=self.sdv_epochs, batch_size=self.sdv_batch_size, verbose=True)
        elif self.method == 'tvae':
            return TVAESynthesizer(metadata=self.metadata, epochs=self.sdv_epochs, batch_size=self.sdv_batch_size)
        elif self.method == 'copula':
            return CopulaGANSynthesizer(metadata=self.metadata, epochs=self.sdv_epochs, batch_size=self.sdv_batch_size, verbose=True)
        else:
            raise ValueError(f"No SDV synthesizer for method '{self.method}'")

    def _validate_custom_distributions(self, custom_distributions: Dict, data: pd.DataFrame) -> Dict:
        ...
        for col, dist in validated_distributions.items():
            if col not in data.columns:
                raise ValueError(f"Column '{col}' specified in custom_distributions does not exist in the dataset.")
            if not isinstance(dist, dict):
                raise TypeError(f"The distribution for column '{col}' must be a dictionary.")
            if not dist:
                self.logger.warning(f"Distribution for column '{col}' is empty. It will be ignored.")
                continue
            ...
            if not math.isclose(total_proportion, 1.0):
                self.logger.warning(f"Proportions for column '{col}' do not sum to 1.0 (sum={total_proportion}). They will be normalized.")
                validated_distributions[col] = {k: v / total_proportion for k, v in dist.items()}
        return validated_distributions

    def _synthesize_sdv(self, n_samples: int, custom_distributions: Optional[Dict] = None) -> pd.DataFrame:
        """Synthesizes data using an SDV model, with support for conditional sampling."""
        self.logger.info(f"Starting conditional SDV synthesis with method: {self.method}...")
        if self.synthesizer is None:
            self.synthesizer = self._get_synthesizer()
            self.synthesizer.fit(self.original_data)
        if not custom_distributions:
            self.logger.info("No custom distributions provided. Generating samples unconditionally.")
            return self.synthesizer.sample(num_rows=n_samples)
        self.logger.info(f"Applying custom distributions via conditional sampling: {custom_distributions}")
        all_synth_parts = []
        if len(custom_distributions.keys()) > 1:
            self.logger.warning(f"Multiple columns found in custom_distributions. Conditioning on first column: '{next(iter(custom_distributions))}'.")
        col_to_condition = self.target_column if self.target_column in custom_distributions else next(iter(custom_distributions))
        ...
        for value, proportion in dist.items():
            ...
            if num_rows_for_val > 0 and remaining_samples > 0:
                ...
                self.logger.info(f"Generating {num_rows_for_val} samples for '{col_to_condition}' = '{value}'")
                try:
                    synth_part = self.synthesizer.sample(num_rows=num_rows_for_val, conditions={col_to_condition: value})
                    all_synth_parts.append(synth_part)
                    remaining_samples -= len(synth_part)
                except Exception as e:
                    self.logger.warning(f"Could not generate conditional samples for {col_to_condition}='{value}': {e}")
        if remaining_samples > 0:
            self.logger.info(f"Generating {remaining_samples} remaining samples unconditionally.")
        ...
        return pd.concat(all_synth_parts, ignore_index=True).sample(frac=1, random_state=self.random_state).reset_index(drop=True)

    def _synthesize_resample(self, n_samples: int, custom_distributions: Optional[Dict] = None) -> pd.DataFrame:
        ...
        if not custom_distributions:
            return self.original_data.sample(n=n_samples, replace=True, random_state=self.random_state)
        self.logger.info(f"Applying custom distributions via weighted resampling: {custom_distributions}")
        self.logger.warning("The 'resample' method with custom distributions changes proportions but does not generate new data.")
        col_to_condition = self.target_column if self.target_column in custom_distributions else next(iter(custom_distributions))
        dist = custom_distributions[col_to_condition]
        ...
        if weights.sum() == 0:
            self.logger.warning("Weights are all zero. Falling back to uniform resampling.")
            return self.original_data.sample(n=n_samples, replace=True, random_state=self.random_state)
        return self.original_data.sample(n=n_samples, replace=True, random_state=self.random_state, weights=weights)

    def _synthesize_gmm(self, n_samples: int, custom_distributions: Optional[Dict] = None) -> pd.DataFrame:
        ...
        if not non_numeric_cols.empty:
            raise ValueError(f"The 'gmm' method only supports numeric data, but found non-numeric columns: {list(non_numeric_cols)}.")
        gmm = GaussianMixture(n_components=self.gmm_n_components, covariance_type=self.gmm_covariance_type, random_state=self.random_state)
        ...
        if self.target_column and self.target_column in synth.columns:
            ...
            if unique_values < 25 or (unique_values / len(self.original_data)) < 0.05:
                self.logger.info(f"Rounding GMM results for target column '{self.target_column}' to nearest integer.")
                synth[self.target_column] = synth[self.target_column].round().astype(int)

        if custom_distributions:
            self.logger.warning("Applying custom distributions to GMM output via post-processing. This may break learned correlations.")
            col_to_condition = self.target_column if self.target_column in custom_distributions else next(iter(custom_distributions))
            dist = custom_distributions[col_to_condition]
            ...
            if len(new_values) < n_synth_samples:
                new_values.extend([list(dist.keys())[-1]] * (n_synth_samples - len(new_values)))
    ...
    def _apply_resampling_strategy(self, X, y, custom_dist, n_samples):
        ...
        try:
            ...
            target_counts = {k: int(v * target_total_size) for k, v in custom_dist.items()}
            oversampling_strategy = {k: v for k, v in target_counts.items() if v > original_counts.get(k, 0)}
            undersampling_strategy = {k: v for k, v in target_counts.items() if v < original_counts.get(k, 0)}
            steps = []
            if oversampling_strategy:
                steps.append(('o', RandomOverSampler(sampling_strategy=oversampling_strategy, random_state=self.random_state)))
            if undersampling_strategy:
                steps.append(('u', RandomUnderSampler(sampling_strategy=undersampling_strategy, random_state=self.random_state)))
            if not steps:
                return X, y
            pipeline = ImblearnPipeline(steps=steps)
            self.logger.info(f"Applying resampling pipeline to match distribution for column '{y.name}'.")
            X_res, y_res = pipeline.fit_resample(X, y)
            return X_res, y_res
        except Exception as e:
            self.logger.warning(f"Could not apply resampling strategy for column '{y.name}': {e}. Using original distribution.")
            return X, y

    def _synthesize_cart(self, data: pd.DataFrame, n_samples: int, custom_distributions: Optional[Dict] = None) -> pd.DataFrame:
        ...
        if custom_distributions:
            self.logger.warning("For 'cart' method, custom distributions are handled by resampling the training data.")
        X_real = data.copy()
        ...
        X_synth = X_real.sample(n=n_samples, replace=True, random_state=self.random_state).reset_index(drop=True)
        try:
            for it in range(self.cart_iterations):
                ...
                for col in X_real.columns:
                    ...
                    # Special heuristic for the main target column
                        ...
                        if unique_values < 25 or (unique_values / len(y_real_train)) < 0.05:
                            is_classification = True
                    ...
                    model = DecisionTreeClassifier(**model_params) if is_classification else DecisionTreeRegressor(**model_params)
                    
                    y_to_fit, X_to_fit = (y_real_train, Xr_real_train)
                    if custom_distributions and col in custom_distributions:
                        X_to_fit, y_to_fit = self._apply_resampling_strategy(Xr_real_train, y_real_train, custom_distributions[col], n_samples)
                    try:
                        model.fit(X_to_fit, y_to_fit)
                    except ValueError as e:
                        if 'Input contains NaN' in str(e):
                            raise ValueError(f"The '{self.method}' method failed due to NaNs. Please pre-clean data.") from e
                        else:
                            raise e
    ...
    def _synthesize_rf(self, data: pd.DataFrame, n_samples: int, custom_distributions: Optional[Dict] = None) -> pd.DataFrame:
        ...
        if custom_distributions:
            self.logger.warning("For 'rf' method, custom distributions are handled by resampling the training data.")
        X_real = data.copy()
        for col in X_real.select_dtypes(include=['object']).columns:
            X_real[col] = X_real[col].astype('category')
        # Sample with replacement and reset index immediately to prevent issues with duplicate indices.
    ...
    def _synthesize_lgbm(self, data: pd.DataFrame, n_samples: int, custom_distributions: Optional[Dict] = None) -> pd.DataFrame:
        ...
        if custom_distributions:
            self.logger.warning("For 'lgbm' method, custom distributions are handled by resampling the training data.")
        X_real = data.copy()
        # Sample with replacement and reset index immediately to prevent issues with duplicate indices.
        X_synth = X_real.sample(n=n_samples, replace=True, random_state=self.random_state).reset_index(drop=True)
        ...
        for col in cat_cols:
            X_synth[col] = pd.Categorical(X_synth[col], categories=X_real[col].cat.categories)
        ...
        try:
            for it in range(self.cart_iterations):
                ...
                for col in X_real.columns:
                    ...
                    model_params = {"random_state": self.random_state, "n_jobs": 1, "verbose": -1}
                    ...
                    if y_real_train.dtype.name == 'category':
                        y_synth = pd.Categorical(y_synth, categories=y_real_train.cat.categories)
                    X_synth[col] = y_synth
            return X_synth
        except Exception as e:
    ...
    def _synthesize_datasynth(self, n_samples: int, custom_distributions: Optional[Dict] = None) -> pd.DataFrame:
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                synth = generator.generate_dataset_in_correlated_attribute_mode(
                    n_samples=n_samples, 
                    description_file=temp_description_file
                )
                
                # Apply custom distributions (Post-processing)
                if custom_distributions:
                    self.logger.warning("Applying custom distributions to DataSynthesizer output via post-processing.")
                    col_to_condition = self.target_column if self.target_column in custom_distributions else next(iter(custom_distributions))
                    dist = custom_distributions[col_to_condition]
                    n_synth_samples = len(synth)
    ...
    def _inject_dates(self, df: pd.DataFrame, date_col: str, date_start: Optional[str], date_every: int, date_step: Optional[Dict[str, int]]) -> pd.DataFrame:
        """Injects a date column into the DataFrame with specified frequency and step."""
        if date_start is None: return df
        ...
        if date_col not in df.columns:
            ...
            df[date_col] = series
        self.logger.info(f"[RealGenerator] Injected date column '{date_col}' starting at {start_ts}.")
        return df
    ...
    def synthesize(
        ...
        """
        self.logger.info(f"Starting synthesis of {n_samples} samples using method '{self.method}'...")
        if custom_distributions:
            custom_distributions = self._validate_custom_distributions(custom_distributions, self.original_data)
        if self.balance_target and self.target_column and (custom_distributions is None or self.target_column not in custom_distributions):
            self.logger.info(f"'balance_target' is True. Generating balanced distribution for '{self.target_column}'.")
            target_classes = self.original_data[self.target_column].unique()
            custom_distributions = custom_distributions or {}
            custom_distributions[self.target_column] = {c: 1/len(target_classes) for c in target_classes}
        try:
            synth = None
            if self.method in ['ctgan', 'tvae', 'copula']:
                synth = self._synthesize_sdv(n_samples, custom_distributions=custom_distributions)
            elif self.method == 'resample':
                synth = self._synthesize_resample(n_samples, custom_distributions=custom_distributions)
            elif self.method == 'cart':
                synth = self._synthesize_cart(self.original_data, n_samples, custom_distributions=custom_distributions)
            elif self.method == 'rf':
                synth = self._synthesize_rf(self.original_data, n_samples, custom_distributions=custom_distributions)
            elif self.method == 'lgbm':
                synth = self._synthesize_lgbm(self.original_data, n_samples, custom_distributions=custom_distributions)
            elif self.method == 'gmm':
                synth = self._synthesize_gmm(n_samples, custom_distributions=custom_distributions)
            elif self.method == 'datasynth':
                synth = self._synthesize_datasynth(n_samples, custom_distributions=custom_distributions)
            
            if synth is not None:
                self.logger.info(f"Successfully synthesized {len(synth)} samples.")
                synth = self._inject_dates(df=synth, date_col=date_col, date_start=date_start, date_every=date_every, date_step=date_step)
                if self.auto_report:
                ...
                if save_dataset: # Only save if save_dataset is True
                    try:
                        save_path = os.path.join(output_dir, f'synthetic_data_{self.method}.csv')
                        synth.to_csv(save_path, index=False)
                        self.logger.info(f"Generated synthetic dataset saved to: {save_path}")
                    except Exception as e:
                        self.logger.error(f"Failed to save synthetic dataset: {e}")

                return synth
            else:
                self.logger.error(f"Synthesis method '{self.method}' failed to generate data.")
                return None
        except Exception as e:
            self.logger.error(f"Synthesis with method '{self.method}' failed: {e}", exc_info=True)
            return None