...
class ClinicGenerator:
    ...
    def generate_demographic_data(self, n_samples: int, 
        ...
        patient_ids = [f'PAT_{np.random.randint(10000, 99999)}_{i}' for i in range(n_samples)]
        ...
        if class_assignment_function:
            ...
            df_temp['Disease_Subgroup'] = np.where(group_final == 1, 'Disease', 'Control')
        ...
        if 'Disease_Subgroup' in raw_demographic_data.columns:
            raw_demographic_data = raw_demographic_data.drop(columns=['Disease_Subgroup'])
        if 'Propensity' in raw_demographic_data.columns:
            raw_demographic_data = raw_demographic_data.drop(columns=['Propensity'])
        if 'Sex' in raw_demographic_data.columns and 'Sex_Binario' in raw_demographic_data.columns:
    ...
    def _prepare_demographic_context(self, demographic_df, demographic_id_col, raw_demographic_data,
        ...
        if demographic_df is not None:
            ...
            groups = demographic_df['Group'].values if 'Group' in demographic_df.columns else np.array(['Control'] * n_samples)
            ...
            patient_ids = np.array([f'PAT_{np.random.randint(10000, 99999)}_{i}' for i in range(n_samples)])
            n_control = int(n_samples * control_disease_ratio)
            n_disease = n_samples - n_control
    ...
    def inject_drift_group_transition(self,
        ...
        def get_patient_ids_for_transition(source_group: str):
            source_patients = updated_demographic_df[updated_demographic_df['Group'] == source_group].index.tolist()
            num_to_transition = 0
        ...
        for patient_id in patient_ids_to_modify:
            ...
            if 'Binary_Group' in updated_demographic_df.columns:
                updated_demographic_df.loc[patient_id, 'Binary_Group'] = 1 if new_group == 'Disease' else 0

        # Regenerate omics data for all transitioned patients
        if patient_ids_to_modify:
            control_to_disease_ids = [pid for pid in patient_ids_to_modify if updated_demographic_df.loc[pid, 'Group'] == 'Disease']
            disease_to_control_ids = [pid for pid in patient_ids_to_modify if updated_demographic_df.loc[pid, 'Group'] == 'Control']
...
def replicate_genes_proteins(generator: ClinicGenerator, mode, output_dir, n_samples=100, factor_escala=1.0, gene_mean_loc_center=7.0, gene_mean_log_center=np.log(80)):
    """
    ...
    df_demo_t1.to_csv(os.path.join(output_dir, f"demographic_dataset_t1_{mode}.csv"), index=True)
    df_genes_t1.to_csv(os.path.join(output_dir, f"genes_dataset_t1_{mode}.csv"), index=True)
    df_proteins_t1.to_csv(os.path.join(output_dir, f"dataset_proteins_t1_{mode}.csv"), index=True)
    ...
    print(f"T2 Cohort: {len(df_demo_t2[df_demo_t2['Group'] == 'Control'])} control, {len(df_demo_t2[df_demo_t2['Group'] == 'Disease'])} disease.")
    ...
    df_demo_t2.to_csv(os.path.join(output_dir, f"demographic_dataset_t2_{mode}.csv"), index=True)
    df_genes_t2.to_csv(os.path.join(output_dir, f"genes_dataset_t2_{mode}.csv"), index=True)
    df_proteins_t2.to_csv(os.path.join(output_dir, f"dataset_proteins_t2_{mode}.csv"), index=True)
