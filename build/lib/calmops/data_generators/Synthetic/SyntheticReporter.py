...
class SyntheticReporter:
    ...
    def generate_report(
        ...
        report = {
            ...
            "target_analysis": self._report_target_analysis(synthetic_df, target_column) if target_column else None,
        ...
        final_time_col = time_col if time_col and time_col in synthetic_df.columns else 'timestamp' if 'timestamp' in synthetic_df.columns else None

        # --- Generate and Save Plots for the entire dataset --- #
        plots = {}
        plots['pca'] = self._save_pca_plot(synthetic_df, output_dir, generator_name)
        plots['correlation_heatmap'] = self._save_correlation_heatmap(synthetic_df, output_dir, focus_cols=focus_cols)
        plots['interaction_plots'] = self._save_interaction_plots(synthetic_df, output_dir, focus_cols=focus_cols)
        plots['mode_analysis'] = self._save_mode_analysis_plot(report["mode_analysis"], output_dir)
        ...
        for col in cols_to_plot:
            if col in synthetic_df.columns:
                if col == final_time_col:
                    continue
                dist_plots[col] = self._save_distribution_plot(synthetic_df, col, output_dir)
            
            is_categorical = pd.api.types.is_string_dtype(synthetic_df[col]) or pd.api.types.is_categorical_dtype(synthetic_df[col]) or \
                             (pd.api.types.is_numeric_dtype(synthetic_df[col]) and synthetic_df[col].nunique() < 25)

            if is_categorical:
                balance_plots[col] = self._save_instances_balance_plot(synthetic_df, col, output_dir)
        ...
        plots['time_evolution_plots'] = self._save_time_evolution_plots(
            synthetic_df, output_dir, time_col=final_time_col, focus_cols=focus_cols, drift_config=drift_config
        )
        ...
        if block_column and block_column in synthetic_df.columns:
            ...
            for block_id in unique_blocks:
                ...
                for col in block_df.columns:
                    ...
                    plot_path = self._save_distribution_plot(block_df, col, block_output_dir)
                    if plot_path:
                        block_plots_report[str(block_id)]["distribution_plots"][col] = plot_path

                    is_categorical = pd.api.types.is_string_dtype(block_df[col]) or pd.api.types.is_categorical_dtype(block_df[col]) or \
                                     (pd.api.types.is_numeric_dtype(block_df[col]) and block_df[col].nunique() < 25)
                    
                    if is_categorical:
                        balance_plot_path = self._save_instances_balance_plot(block_df, col, block_output_dir)
    ...
    def _report_block_analysis(self, synthetic_df: pd.DataFrame, block_column: str, target_column: Optional[str]) -> Dict[str, Any]:
        ...
        for block_id in unique_blocks:
            ...
            if target_column and target_column in block_df.columns:
                block_stats["target_distribution"] = block_df[target_column].value_counts(normalize=True).to_dict()
            
            analysis[str(block_id)] = block_stats
        
        return analysis

    def _save_instances_balance_plot(self, df: pd.DataFrame, column: str, output_dir: str) -> Optional[str]:
    ...
    def _save_time_evolution_plots(self, df: pd.DataFrame, output_dir: str, time_col: Optional[str] = None, focus_cols: Optional[List[str]] = None, drift_config: Optional[Dict[str, Any]] = None) -> Dict[str, str]:
        ...
        for col in features_to_plot:
            if col == time_col or col == 'block': continue
        ...
        for col in numeric_cols:
            try:
                ...
                if time_col:
                    ...
                    mean_ma = agg['mean'].rolling(window=moving_avg_window, min_periods=1, center=True).mean()
                    mean_ma.plot(ax=ax, label=f'Mean ({moving_avg_window}-MA)', style='-', linewidth=2.5, color=color)

                else:
                    ma = df[col].rolling(window=moving_avg_window, min_periods=1, center=True).mean()
                    ma.reset_index(drop=True).plot(ax=ax, label=f'({moving_avg_window}-MA)', style='-', linewidth=2, color=color)

                if drift_config: pass
        ...
        if time_col:
            for col in categorical_cols:
                plots[col] = self._save_categorical_evolution_plot(df, time_col, col, output_dir)
        
        return plots

    def _save_categorical_evolution_plot(self, df: pd.DataFrame, time_col: str, category_col: str, output_dir: str) -> Optional[str]:
        ...
        try:
            ...
            if n_unique > HIGH_CARDINALITY_THRESHOLD:
                self.logger.info(f"High cardinality ({n_unique}) for '{category_col}', grouping less frequent categories.")
                top_categories = df_plot[category_col].value_counts().nlargest(HIGH_CARDINALITY_THRESHOLD - 1).index.tolist()
                df_plot[category_col] = df_plot[category_col].apply(lambda x: x if x in top_categories else 'Other')

            pivot = df_plot.pivot_table(index=time_col, columns=category_col, aggfunc='size', fill_value=0)
            pivot = pivot.div(pivot.sum(axis=1), axis=0)

            fig, ax = plt.subplots(figsize=(16, 8)) # Increased figure size for better readability
            ...
            ax.legend(title=category_col, bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=12)
            ax.grid(True, which='major', linestyle='--', linewidth='0.5', color='grey')
    ...
    def _save_distribution_plot(self, df: pd.DataFrame, column: str, output_dir: str) -> Optional[str]:
        ...
        try:
            ...
            if pd.api.types.is_numeric_dtype(df[column]):
                ...
                if len(df_counts) > 10:
                    ...
                    df_counts.plot(kind='line', alpha=0.8, ax=ax, marker='o', linewidth=2, markersize=8)
    ...
    def _save_correlation_heatmap(self, df: pd.DataFrame, output_dir: str, focus_cols: Optional[List[str]] = None) -> Optional[str]:
        """Saves a heatmap of the correlation matrix for numeric features."""
        try:
            numeric_cols = focus_cols if focus_cols else df.select_dtypes(include=np.number).columns.tolist()
    ...
    def _save_interaction_plots(self, df: pd.DataFrame, output_dir: str, max_plots: int = 5, focus_cols: Optional[List[str]] = None) -> Dict[str, str]:
        ...
        try:
            ...
            for num_col in numeric_cols:
                for cat_col in categorical_cols:
                    if num_col == cat_col or plot_count >= max_plots: continue
    ...
    def _save_pca_plot(self, df: pd.DataFrame, output_dir: str, generator_name: str) -> Optional[str]:
        ...
        try:
            ...
            categorical_features = df_pca.select_dtypes(include=['object', 'category']).columns.tolist()
    ...
    def _report_target_analysis(self, df: pd.DataFrame, target_column: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if pd.api.types.is_string_dtype(target) or pd.api.types.is_categorical_dtype(target) or target.nunique() < 25:
    ...
    def _save_boxplot_plot(self, df: pd.DataFrame, column: str, output_dir: str) -> Optional[str]:
    ...
    def _save_mode_analysis_plot(self, mode_analysis_data: Dict[str, Any], output_dir: str) -> Optional[str]:
        ...
        try:
            ...
            proportions = [d.get('mode', {}).get('proportion', 0) for d in mode_analysis_data.values()]
            modes = [str(d.get('mode', {}).get('value', 'N/A')) for d in mode_analysis_data.values()]

            df = pd.DataFrame({'Feature': columns, 'Proportion': proportions, 'Mode': modes})
            ...
            labels = [f"{df['Mode'][i]} ({df['Proportion'][i]:.2f})" for i in range(len(bars))]
