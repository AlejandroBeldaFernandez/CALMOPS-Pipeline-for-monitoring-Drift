# pipeline_block/modules/evaluator.py
...
def evaluate_model(
    ...
    *,
    ...
    if (
        hasattr(model, "route_by_block")
        or hasattr(model, "per_block_models")
        or hasattr(model, "global_model")
    ):
    ...
    results = {
        ...
        "model_info": {
            ...
            "is_router": bool(
                hasattr(model, "per_block_models") or hasattr(model, "route_by_block")
            ),
        },
    }
    _save_json(results, metrics_dir, "eval_results.json")
