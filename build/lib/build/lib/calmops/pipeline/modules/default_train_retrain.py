...
def default_train(
...
) -> Tuple[Any, pd.DataFrame, pd.Series, dict]:
    ...
    if (
        "torch" in str(type(model_instance)).lower()
        or "skorch" in str(type(model_instance)).lower()
    ):
...
def default_retrain(
    ...
    def _safe_train_return(
        _model, _X_train, _y_train, _X_eval, _y_eval, extra_info=None
    ):
    ...
    def _split(X_, y_):
        ...
        return train_test_split(
            X_, y_, test_size=0.2, random_state=random_state, stratify=strat
        )
    ...
    try:
        ...
        # === Mode 5: Replay mix (blend previous_data.csv with current) ===
            ...
            control_dir = os.path.join(
                os.path.dirname(os.path.dirname(output_dir)), "control"
            )
            prev_csv = os.path.join(control_dir, "previous_data.csv")
            if not os.path.exists(prev_csv):
                logger.warning(
                    "Previous training data not found; falling back to full retraining on current data"
                )
            ...
            try:
                ...
                tgt = (
                    y.name
                    if (hasattr(y, "name") and y.name in df_prev.columns)
                    else None
                )
                if tgt is None:
                    raise ValueError(
                        "Target column not found in previous_data.csv for replay mix."
                    )
        ...
        # === Mode 6: Recalibration only (classification) ===
            ...
            if not is_classifier(base):
                raise ValueError(
                    "Mode 6 (recalibration) is only valid for classification models."
                )

            results["strategy"] = (
                "probability calibration (Platt/Isotonic) with base prefit"
            )
            ...
            def _make_calibrator(_base, method):
                try:
                    return CalibratedClassifierCV(
                        estimator=_base, method=method, cv="prefit"
                    )
                except TypeError:
                    return CalibratedClassifierCV(
                        base_estimator=_base, method=method, cv="prefit"
                    )
