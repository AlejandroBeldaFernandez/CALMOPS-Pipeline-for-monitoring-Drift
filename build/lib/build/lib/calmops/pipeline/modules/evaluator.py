...
def _upsert_control_entry(
    ...
    if logger:
        logger.info(
            f"Control file updated: {key_name} registered with modification time {mtime}"
        )
...
def save_eval_results(
    ...
    if results.get("approved", False):
        ...
        historical_eval_path = os.path.join(
            eval_history_dir, f"eval_results_{timestamp}.json"
        )
        with open(historical_eval_path, "w") as f:
            json.dump(serializable_results, f, indent=4)
        if logger:
            logger.info(
                f"Historical evaluation results saved to {historical_eval_path}"
            )
...
def _save_candidate(
    ...
    meta = {
        ...
        "metrics_keys": list(results.get("metrics", {}).keys())
...
def evaluate_model(
    *,
    ...
    user_type = (
        "classification"
        if (user_keys & classification_metrics)
        else "regression"
        if (user_keys & regression_metrics)
        else None
    )
    ...
    if user_type == "classification" and (user_keys & regression_metrics):
        raise ValueError(
            "Configuration error: Cannot mix classification and regression metrics in thresholds_perf."
        )
    if user_type == "regression" and (user_keys & classification_metrics):
        raise ValueError(
            "Configuration error: Cannot mix regression and classification metrics in thresholds_perf."
        )
    if is_classification and user_type == "regression":
        raise ValueError(
            "Model type mismatch: Classification model detected but thresholds_perf contains regression metrics."
        )
    if (not is_classification) and user_type == "classification":
        raise ValueError(
            "Model type mismatch: Regression model detected but thresholds_perf contains classification metrics."
        )
    ...
    if not is_first_model:
        ...
        if os.path.exists(champion_model_path):
            try:
                ...
                champion_metrics, _ = calculate_metrics(
                    champion_model, X_test, y_test, is_classification
                )
                logger.info("Champion model loaded and evaluated for comparison.")
            except Exception as e:
                logger.warning(
                    f"Could not load or evaluate Champion model for comparison: {e}"
                )
        else:
            logger.info(
                "No Champion model found for comparison (expected if first model, unexpected otherwise)."
            )

    try:
        metrics, y_pred, y_pred_proba = calculate_metrics(
            model, X_test, y_test, is_classification
        )

        # Keep a small sample of predictions to aid debugging
        sample_df = pd.DataFrame({"y_true": y_test, "y_pred": y_pred})
        if y_pred_proba is not None:
            for i in range(y_pred_proba.shape[1]):
                sample_df[f"y_pred_proba_{i}"] = y_pred_proba[:, i]

        sample_predictions = sample_df.sample(
        ...
        for metric, threshold in thresholds_perf.items():
            ...
            if value is None:
                logger.warning(
                    f"Metric '{metric}' could not be calculated, skipping threshold check"
                )
                continue
            ...
            if metric in {"rmse", "mae", "mse"}:
                if value > threshold:
                    logger.warning(
                        f"Threshold violation: {metric}={value:.6f} exceeds maximum allowed threshold of {threshold}"
                    )
                    approved = False
                else:
                    logger.info(
                        f"Threshold passed: {metric}={value:.6f} <= {threshold}"
                    )
            elif metric == "r2":
                if value < threshold:
                    logger.warning(
                        f"Threshold violation: {metric}={value:.6f} below minimum required threshold of {threshold}"
                    )
                    approved = False
                else:
                    logger.info(
            ...
            if (
                ...
                and metric in champion_metrics
                and min_improvement_thresholds
            ):
                champion_value = champion_metrics.get(metric)
                min_improvement_ratio = min_improvement_thresholds.get(
                    metric, 0.10
                )  # Default to 10%
                ...
                if champion_value is not None and min_improvement_ratio is not None:
                    if metric in {"rmse", "mae", "mse"}:  # Lower is better
                        ...
                        if value > required_value:
                            logger.warning(
                                f"Improvement check violation: {metric}={value:.6f} is not {min_improvement_ratio:.0%} better than Champion's {champion_value:.6f} (required <= {required_value:.6f})"
                            )
                            approved = False
                        else:
                            logger.info(
                                f"Improvement check passed: {metric}={value:.6f} is {min_improvement_ratio:.0%} better than Champion's {champion_value:.6f}"
                            )
                    elif metric in {
                        "accuracy",
                        "balanced_accuracy",
        ...
        results = {
            ...
            "approved": approved,
            "model_role": "Champion" if approved else "Challenger",
            "metrics": metrics,
    ...
    if approved:
        try:
            ...
            # It's safer to use the columns from X_test to select features from df
            ...
            predictions_path = (
                Path(output_dir) / f"predictions_{Path(last_processed_file).stem}.csv"
            )
            ...
            if dir_predictions:
                try:
                    ...
                    logger.info(f"Full predictions also saved to {extra_pred_path}")
                except Exception as e:
                    logger.warning(
            ...
            logger.error(
                f"Critical error during model deployment or prediction saving: {e}"
            )
            # Model deployment failed but evaluation results are still persisted

    else:
        logger.info(
            "Model rejected - performance thresholds not met, champion remains unchanged"
        )
